...
%% Added varying solar power. Uses sliding/embedded mode
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
h(4)=subplot(4,1,4);
plot(T,LoadRef2(T),'displayname','Reference Load Profile [kW]');
ylabel('Power [kW]')
xlabel('Time [Hr]');
...
T= t0:hopt:tf-hopt;
h(5) = subplot(3,1,1);
stairs(T_save(2:end),u_save(1,:),'displayname','Uess [kW]');
legend('show');

h(6) = subplot(3,1,2);
stairs(T_save(2:end),u_save(2,:),'displayname','Udiesel [kW]');
legend('show');

h(8) = subplot(3,1,3);
stairs(T_save(2:end),v_save,'displayname','Mode');
stairs(T_save(2:end),u_save(1,:)<=0,'displayname','Mode');
legend('show');
axis([0 24 -0.1 1.1])
linkaxes(h,'x')
